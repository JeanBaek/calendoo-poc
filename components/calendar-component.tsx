                ...
                <ul className="space-y-4">
                    {events && (
                        <li key={"create"} className="p-4 border rounded shadow">
                            <h2 className="text-xl font-bold">{events.success && '성공적으로 이벤트 생성! 🎉'}</h2>
...
export const EventDeleteComponent = () => {
    ...
    const createCalendarEvent = async () => {
        ...
        const data: CreateRes = await response.json();
        console.log('Events ', data.success);
        ...
        <>
            ...
            {events && (
                <ul className="space-y-4">
                    {events && (
                        <li key={"create"} className="p-4 border rounded shadow">
                            <h2 className="text-xl font-bold">{events.success && '성공적으로 이벤트 업데이트! 🎉'}</h2>
                            <p className="text-gray-600">{events.htmlLink}</p>
                        </li>
                    )}
                </ul>
